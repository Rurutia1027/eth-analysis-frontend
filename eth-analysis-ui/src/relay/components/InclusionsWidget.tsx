...
const PayloadRow = ({ blockNumber, insertedAt, value }: Payload) => {
  ...
  const inclusionAgo =
    now === undefined ? undefined : Format.formatDurationToNow(now, insertedAt);
...
const InclusionsWidget: FC<Props> = ({
  ...
  const totalValueFormed = Format.formatTwoDigit(totalValue); 
    ...
    <div className="flex flex-col gap-y-4">
      <WidgetBackground>
        ...
        <div className={`
          ...
          <div>
            <span className="text-slateus-200">{`${totalValueFormed} ETH`}</span>
